...
const CAPABILITIES = '/pub/pubky-nextjs-template/:rw';

const HTTP_NEXUS = process.env.NEXT_PUBLIC_HTTP_NEXUS || 'https://nexus.pubky.app/';
...
async function getHomeserverFor(session: any) {
  ...
  try {
    ...
    return { success: true, homeserverKey: homeserver.z32(), pkarrRelays: PKARR_RELAYS, addresses, port };
  } catch (error) {
